-- Widget (Campus Director View): Past Contract Grad Date
-- Written by: ??
-- 6/8/2018 Update (Kelly)
   -- Now accounts for 'Daily' or 'Hourly' surcharges
   -- Logic: code calculates average of lesson durations for each student, then divides overall excess hours by the average to find the number of 'days' the student has missed.
-- 8/10/2018 Update (Kelly)
   ...
   -- Filtered list by current user's campus
...
WHERE
  ...
  AND A.<ADMINID>
  AND S.studentCampus = (SELECT SA.campusCode FROM SubAdmins SA WHERE SA.subAdminId = [USERID] AND SA.<ADMINID>)
